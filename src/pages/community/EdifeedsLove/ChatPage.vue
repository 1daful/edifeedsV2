    ...
    <div class="messages-container" ref="messagesContainer">
      <div class="messages-list">; 
        <div class="date-separator">
          <span class="date-text">{{ date.formatDate(new Date()) }}</span>
        </div>
      ...
      <div class="input-container">
        ...
        <transition name="send-btn">
          <q-btn
            ...
            round color="primary" icon="send" class="send-btn" @click="sendMessage"
          />
...
const isOtherTyping = computed(() => chat.getTypingUsers(matchId).value.includes(otherUserId))
const isOnline = computed(() => chat.getPresence(matchId).value.some(p => p.userId === otherUserId))
...
// Add this watch function in your <script setup> block
watch(messages, () => {
    // A new message has been added, mark it as read
    chat.markRead(matchId);
}, { deep: true });
...
onMounted(async () => {
  try {
    ...
    if (user) {
      ...
      const userProfile = { name: profile?.display_name, avatar: profile?.avatar_url, userId: user.id };
