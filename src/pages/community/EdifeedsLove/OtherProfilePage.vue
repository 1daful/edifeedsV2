<template>
  <div class="q-pa-md" v-if="profileUser">
    <q-card class="profile-overview-card">
      <div class="relative-position">
        ...
        <div class="bg-gradient-dark text-white q-pa-md">
          <div class="text-h5 text-weight-bold">{{ profileUser.display_name || profileUser.name }}, {{ profileUser.age }}</div>
    ...
    <div class="q-mt-md">
      <q-list bordered class="rounded-borders">
        <router-link :to="{
          ...
          query: { otherUserId: match.other_user_id },
        }" v-if="canMessage">
          <q-item clickable>
...
const route = useRoute()
  ...
  const profileUser = JSON.parse(route.params.otherUser)

  const getMatch = computed(() => {
    const matchStore = useMatchStore()
    const match = matchStore.matches.find(m => m.other_user_id === profileUser.id)
    const favourite = matchStore.favourites.find(f => f.other_user_id === profileUser.id)
    const admirer = matchStore.admirers.find(a => a.other_user_id === profileUser.id)
    const isMatched = (match || favourite || admirer) ? true : false
    console.log("ADMIRER: ", admirer)
    return { match, favourite, admirer, isMatched }
  })

  const match = computed(() => {
    return getMatch.value.match || getMatch.value.favourite || getMatch.value.admirer || { id: null }
  })

  const distanceFromUser = route.query.distance
  const showPublicStats = ref(true)
...
// Computed properties
const canMessage = computed(() => getMatch.value.isMatched)
...
const sendMessage = () => {
  emit('message-sent', profileUser)
}

const viewPhotos = () => {
  emit('photos-viewed', profileUser)
}

const shareProfile = () => {
  emit('profile-shared', profileUser)
}

const reportProfile = () => {
  emit('profile-reported', profileUser)
  showMoreDialog.value = false
}

const blockUser = () => {
  emit('user-blocked', profileUser)
  showMoreDialog.value = false
}
...
onMounted(() => {
  ...
  console.log('Favourite:', useMatchStore().favourites)

  console.log('otherUser:', route.params.otherUser)
  console.log('All matches:', getMatch.value)
})
</script>
