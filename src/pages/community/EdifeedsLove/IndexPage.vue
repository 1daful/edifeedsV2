      ...
      <q-page>
        ...
        <q-tab-panels v-model="tab" animated transition-prev="slide-right" transition-next="slide-left">
              ...
              >
                <q-card
                  ...
                  @click="router.push({
                    ...
                    //path: 'visitor/' + user.id,
                    params: { otherUser: JSON.stringify(user) }
                    });"
            ...
            <div class="q-pa-md">
              ...
              <q-scroll-area style="height: 200px" class="q-mb-lg">
                <div class="row no-wrap q-gutter-md">
                  <div
                    v-for="match in favourites"
                    :key="'recent-' + match.id"
                    class="col-auto"
                  >
                    <q-card class="recent-match-card cursor-pointer" @click="router.push({
                      name: 'edifeeds-love-profile',
                      //path: 'visitor/' + match.otherUserId,
                      params: { otherUser: JSON.stringify(match) }
                      })">
              ...
              <div class="row q-col-gutter-md">
                <div
                  ...
                  class="col-12 col-sm-6 col-md-4"
                >
                  <q-card class="match-card cursor-pointer" @click="router.push({ name: 'edifeeds-love-profiles', params: { otherUser: match } })">
              ...
              <div class="row q-col-gutter-md">
                ...
                >
                  <q-card class="like-card">
                    <div class="relative-position">
                      <q-img :src="admirer.avatar" ratio="1" class="rounded-t-borders blur-img" />
                      <div class="absolute-full flex flex-center bg-dark bg-opacity-70 text-white">
                        <div class="text-center">
                          ...
                          <div class="text-caption">See who admires you</div>
                        </div>
                      </div>
                    </div>
              ...
              <PremiumAdmirerView :admirers="admirers" />
            </div>
          </q-tab-panel>
          ...
          <q-tab-panel name="profile">
            <ProfileCard :currentUser="currentUser" :matchCount="profileStats.matches" />
          </q-tab-panel>
        </q-tab-panels>
      </q-page>
    </q-page-container>
...
import ProfileCard from "../../../components/Community/Profile.vue";
import PremiumAdmirerView from "../../../components/Community/PremiumAdmirerView.vue";
...
const conversations = ref([
  {
    id: '9524aee4-6b04-4777-a997-e6819d39c7da',
    //id: '9524aee4-6b04-4777-a997-e6819d39c7da'
    name: "Grace",
    age: 27,
    avatar: "https://source.unsplash.com/400x400/?woman,portrait",
    ...
    //isTyping: false,
    messages: [
      { id: 1, text: "Hey! How was your weekend at church?", timestamp: "10:30 AM", sent: false, sender_id: 45, receiver_id: 1, read: true },
      { id: 2, text: "It was incredible! We had a guest speaker who talked about missions in Africa", timestamp: "10:32 AM", sent: true, sender_id: 1, receiver_id: 45, read: true },
      { id: 3, text: "That sounds amazing! I've always felt called to missions too", timestamp: "10:35 AM", sent: false, sender_id: 45, receiver_id: 1, read: false },
      { id: 4, text: "Really? What field are you most interested in?", timestamp: "10:36 AM", sent: true, sender_id: 1, receiver_id: 45, read: true },
      { id: 5, text: "I'm passionate about education and children's ministry. Teaching kids about God's love", timestamp: "10:40 AM", sent: false, sender_id: 45, receiver_id: 1, read: false },
      { id: 6, text: "That's so beautiful! Children have such pure hearts ❤️", timestamp: "10:42 AM", sent: true, sender_id: 1, receiver_id: 45, read: true },
      { id: 7, text: "Exactly! Their faith is so genuine and inspiring", timestamp: "10:45 AM", sent: false, sender_id: 45, receiver_id: 1, read: false },
      { id: 8, text: "That's amazing! I'd love to hear more about your mission trip. Are you planning any more in the future?", timestamp: "2 min ago", sent: false, sender_id: 45, receiver_id: 1, read: false }
    ]
  },
  {
    id: 2,
...
async function likeUser(targetUserId) {
  ...
  if (error) {
    ...
    console.log('Like recorded successfully', admirers.value, favourites.value);

    $q.notify({
      message: 'You liked ' + targetUserId,
      color: 'green',
      position: 'top-right'
    })
  }
...
const timeAgo = (date2) => {
      ...
      const now = new Date();
      const pastTime = new Date(date2);

      const diffInDays = date.getDateDiff(now, pastTime, 'days');
      const diffInHours = date.getDateDiff(now, pastTime, 'hours');
      const diffInMinutes = date.getDateDiff(now, pastTime, 'minutes');
...
onMounted(async () => {
  ...
  let realProfiles = await dbClient.get({
      url: 'dating_profiles_with_presence',
      params: {
        /*columns: [
          foreignColumns("user_presence", [
            "online",
            "last_seen"
          ]),
        ],*/
        filters: [
          {
            col: 'id',
            op: 'neq',
            val: (await supabase.auth.getUser()).data.user?.id
          }
        ],
        modifiers: [
          {
    ...
    realProfiles = realProfiles.map(profile => ({
      ...
      online: Date.now() - new Date(profile.last_seen).getTime() < 120_000,
    ...
    profiles.value.push(...realProfiles);

    const { data, error } = await supabase
  .rpc("get_user_matches_with_messages", { current_user_id: user.value?.id });

  console.log(error)
  matchesAndMessages.value = data || []

  data.forEach(match => {
    const messages = match.messages || []
    const lastMessageContent = messages[messages.length - 1]?.content || '';
    const unreadCount = messages.filter(msg => !msg.read && msg.sender_id === match.id).length;
    if (match.status === "accepted") {
    ...
    else if (match.status === "pending" && match.initiator_id !== user.value.id) {
      // Incoming like
      admirers.value.push({
        ...match,
        preview: `${match.display_name} liked you!`
      });
    }
      ...
      favourites.value.push({
        ...
        online: Date.now() - new Date(match.last_seen).getTime() < 120_000,
        avatar: match.avatar_url,
        matchTime: timeAgo(match.created_at) || '',
      });
    }
    ...
    if (messages.length > 0) {
      conversations.value.push({
        ...
        messages: messages.map(msg => ({
          ...
          read: msg.read,
          sent: () => {if (msg.receiver_id === user.value.id) { return true; } else { return false; }},
    ...
    /*matches.value.push({
      ...
      age: calculateAge(match.date_of_birth),
      lastMessage: lastMessageContent || '',
      lastMessageTime: messages[messages.length - 1]?.created_at || '' ? date.formatDate(messages[messages.length - 1].created_at) : "",
      unreadMessages: unreadCount,
      online: Date.now() - new Date(match.last_seen).getTime() < 120_000
    });*/
 });
 ...
 /*matchStore.$patch(state => {
  // This is the correct way to update the state.
  // The 'state' parameter is a reference to the store's state.
  state.favourites = favourites.value;
  state.admirers = admirers.value;
  state.matches = matches.value;
  state.conversations = conversations.value;
});*/

matchStore.setAdmirers(admirers.value);
matchStore.setFavourites(favourites.value);
matchStore.setMatches(matches.value);
matchStore.setConversations(conversations.value);

console.log("USER ID:", (await supabase.auth.getUser()).data.user?.id);
console.log('favorites: ', favourites.value, 'admirers: ', admirers.value, 'matches: ', matches.value, 'conversations: ', conversations.value);
