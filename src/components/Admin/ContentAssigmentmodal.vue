<template>
  <q-dialog v-model="show" @show="onDialogShow">
    <q-card>
      <q-card-section class="row items-center q-pb-none">
        <div class="col">
          <div class="text-h6">Assign Content to Topic</div>
          <div class="text-subtitle2 text-grey-7">{{ topic.title }}</div>
          <div class="text-caption text-grey-6" v-if="topic.description">
            {{ topic.description }}
          </div>
        </div>
        <q-btn
      ...
      <q-card-actions align="between" class="q-pa-md">
        <div class="text-body2 text-grey-7">
          <span v-if="selected.length > 0">
            Ready to assign {{ selected.length }} item{{ selected.length > 1 ? 's' : '' }}
          </span>
        </div>
...
const emit = defineEmits(['update:modelValue', 'assigned', 'error'])
